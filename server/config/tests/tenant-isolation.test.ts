...
test("tenant guard isolates reads and writes across organizations", { skip: !enabled }, async () => {
  ...
  try {
    await runWithTenant({ organizationId: alpha.id, organizationSlug: alpha.slug }, () =>
      prisma.user.create({
        data: { name: "Shared Email Alpha", email: "shared@example.test", passwordHash: "test", role: "admin" },
      })
    );
    await runWithTenant({ organizationId: beta.id, organizationSlug: beta.slug }, () =>
      prisma.user.create({
        data: { name: "Shared Email Beta", email: "shared@example.test", passwordHash: "test", role: "admin" },
      })
    );

    const alphaUsers = await runWithTenant(
      { organizationId: alpha.id, organizationSlug: alpha.slug },
      () => prisma.user.findMany({ where: { email: "shared@example.test" } })
    );
    ...
    const crossTenantLookup = await runWithTenant(
      { organizationId: alpha.id, organizationSlug: alpha.slug },
      () => prisma.user.findUnique({ where: { id: betaUsers[0].id } })
    );
