...
export default function HomePage() {
  ...
  return <main ref={pageRef} style={brandStyle} className="min-h-screen overflow-hidden bg-[#f8fafc] text-[#0f172a]">
    <nav className="relative z-50 mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
      ...
      <div className="hidden items-center gap-8 text-sm text-[#526158] md:flex"><a href="#platform">Platform</a><a href="#how">How it works</a><a href="#trust">Trust</a></div>
    ...
    <section className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 pb-24 pt-14 lg:grid-cols-[1.02fr_.98fr] lg:px-8 lg:pb-32 lg:pt-24">
      <div className="relative z-10"><div className="mb-7 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white/70 px-3.5 py-2 text-xs font-medium text-[var(--brand)]"><Sparkles className="size-3.5"/>Built for institutions that listen</div>
        <h1 className="max-w-3xl text-5xl font-semibold leading-[.98] tracking-[-.055em] sm:text-6xl lg:text-[5.2rem]">Turn every concern into <span className="text-[var(--brand)]">meaningful action.</span></h1>
        <p className="mt-7 max-w-xl text-lg leading-8 text-[#5a685f]">Resolve gives students, staff and leadership one transparent system to raise issues, coordinate responses and build a more accountable institution.</p>
        <div className="mt-9 flex flex-col gap-3 sm:flex-row"><a href="#demo" className="inline-flex items-center justify-center gap-2 rounded-full bg-[var(--brand)] px-6 py-3.5 font-medium text-white shadow-xl">See {settings.organizationName} in action <ArrowRight className="size-4"/></a><Link href="/login" className="inline-flex items-center justify-center rounded-full border border-slate-300 bg-white/70 px-6 py-3.5 font-medium">Open the platform</Link></div>
    ...
    <section id="demo" className="bg-blue-100 px-5 py-24 lg:px-8"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.9fr_1.1fr]"><div><p className="text-sm uppercase tracking-[.18em] text-[var(--brand)]">See it in action</p><h2 className="mt-4 max-w-lg text-4xl font-semibold tracking-[-.045em] sm:text-5xl">Make listening part of how your institution works.</h2><p className="mt-5 max-w-md leading-7 text-slate-600">Tell us about your organization. We’ll show you how {settings.organizationName} can fit your teams and workflows.</p><div className="mt-8 flex items-center gap-3 text-sm"><Clock3 className="size-5"/>A focused, no-pressure walkthrough</div></div>
    ...
    <footer className="bg-[var(--brand-dark)] px-5 py-8 text-slate-300"><div className="mx-auto flex max-w-7xl flex-col gap-4 text-sm sm:flex-row sm:justify-between"><span className="font-medium text-white">{settings.organizationName}</span><p>Clear concerns. Accountable teams. Better institutions.</p>{settings.supportEmail ? <a href={`mailto:${settings.supportEmail}`} className="text-white">{settings.supportEmail}</a> : <Link href="/login" className="text-white">Platform sign in</Link>}</div></footer>
  </main>
}
